...
=======
library(rcompanion)
>>>>>>> 0c941045f7c9d26943453b669473cb0e42bc79c7
```
...
```
## Datenvorbereitung
Wegen der Erkenntnisse aus der Korrelationsanalyse werden einige Spalten aus den Daten entfernt. Zusätzlich werden unnötige spalten ('id, date') entfernt. 
Außerdem werden doppelt enthaltene Daten entfernt ('sqft_living15,sqft_lot15').
...
Die Daten werden im nächsten Schritt skaliert.

<<<<<<< HEAD
```{r}
data = as_tibble(scale(data))
```
 

Die Daten werden jetzt in Test- und Trainingsdaten aufgeteilt.
=======
...
Das dritte Modell ist ein Neural Network. Wie beim Random Forest werden mehere Settings durchprobiert:
...
<<<<<<< HEAD
Als Absicherung wurden auch die R^2-Werte für die beiden als "besten" in Frage kommenden Modelle betrachtet und auch hier gewinnt `r rownames(res)[1]`:
```{r}
res = t(t(sort(c(`Lineare Regression` = summary(model_r_linearModel)$r.squared,
               `Random Forest` = mean(model_r_randomForest$rsq)))))
colnames(res) = "R-Squared"
round(res, 3)
```


Das beste Modell wird abgespeichert:
